...
function startAV(exps) {
    ...
    var arr = av.ds.array(mysplit(exps[0]));
    var prevArr;
    arr.css(true, defaultCellStyle);
    arr.css( function(x) { return arr.value(x).length === 1; }, 
	     oneCharWidth);
    av.umsg("The initial \u03BB-expression:");
    av.displayInit();

    for(var slide=1; slide<exps.length; slide++) {
	av.clearumsg();
	av.umsg("after beta-reduction #" + slide, {preserve: true});
	if (prevArr) {
	    prevArr.hide();
	}
	prevArr = arr;
	arr = av.ds.array(mysplit(exps[slide]),{style: "plain"});
	arr.css(true, defaultCellStyle);
	arr.css(  function(x) { return arr.value(x).length === 1; }, 
		  oneCharWidth);
	av.step();
    }
    av.label("The lambda expression above is in beta-normal form.");
    av.recorded();
}
